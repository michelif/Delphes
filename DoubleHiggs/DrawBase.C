...
//  label_bonus->Draw("same");
  ...
  if(saveAlsoAsMacro_){
    std::string canvName_C = canvName + ".C";
    c1->SaveAs(canvName_C.c_str());
  }
...
//    if( dataFile_.file!=0 ) totalNfiles++;
    ...
    if( !lumiOnRightSide_ )
      label_cms->Draw("same");
    label_sqrt->Draw("same");
    if( scaleFactor_<0 && !noDATA && !noStack_ )
  ...
  if(saveAlsoAsMacro_){
    std::string canvName_C = canvasName + ".C";
    c1->SaveAs(canvName_C.c_str());
  }
...
//label_algo->AddText(jetAlgoName.c_str());
  ...
  if(saveAlsoAsMacro_){
    std::string canvName_C = outputdir_+"/" + name + ".C";
    c1->SaveAs(canvName_C.c_str());
  }
...
void DrawBase::save_AlsoAsMacro(){
  saveAlsoAsMacro_=true;
}

void DrawBase::notSave_AlsoAsMacro(){
  saveAlsoAsMacro_=true;
}
